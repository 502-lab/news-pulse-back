...
**Independent Test**: 음성 목록 조회 → 미리듣기 URL 확인 → voiceId·readMode 저장 → GET 프로필로 반영 확인. TTS 생성 없이 단독 검증 가능.
...
3. **Given** 인증된 사용자, **When** voiceId=하린, readMode=LISTEN 으로 설정 저장, **Then** 이후 GET 프로필에서 동일 값이 반환된다
...
인증된 사용자가 "오늘의 브리핑"을 요청하면, 003 개인화 피드 상위 N건의 기사 요약을 이어붙인 단일 오디오와 재생 순서 목록(큐)을 받는다. 브리핑 오디오는 하루 단위로 캐시되어 같은 날 재요청 시 즉시 반환된다.

**Why this priority**: 뉴스 청취 앱의 대표 UX. P1 기사 TTS 이후 자연스러운 확장.

**Independent Test**: 브리핑 요청 → 큐(articleIds, 순서) + audioUrl 반환 → 같은 날 재요청 시 동일 audioUrl 즉시 반환(캐시). 003 피드 API와 독립적으로 테스트 가능(최소 기사 수를 직접 삽입).

**Acceptance Scenarios**:

1. **Given** 관심사 설정된 인증 사용자, **When** 오늘의 브리핑 요청, **Then** 상위 N건 기사 ID 목록(재생 큐)과 status=PENDING 반환
2. **Given** 브리핑 생성 완료, **When** 상태 조회, **Then** status=READY + audioUrl + durationSec + 기사 목록 반환
3. **Given** 오늘 이미 생성된 브리핑, **When** 같은 날 동일 사용자가 재요청, **Then** 캐시된 오디오 즉시 반환 (새 생성 없음)
...
- 브리핑 생성 대상 기사가 0건(관심사 없음 + 피드 결과 없음) → 빈 브리핑 또는 에러 반환
- 동일 조합 TTS가 PROCESSING 중일 때 재요청 → 기존 작업 반환(중복 생성 금지)
- audioUrl 만료(CDN TTL) → 재조회 시 새 서명 URL 발급 또는 만료 전 충분한 TTL 설정
...
- **FR-002**: 사용자는 선호 음성(voiceId)과 콘텐츠 소비 방식(readMode: READ/LISTEN/BOTH)을 프로필에 저장할 수 있어야 한다
- **FR-003**: 시스템은 002 reading_preferences를 voiceId·readMode 필드로 확장해야 한다
...
- **FR-010**: 브리핑은 003 개인화 피드 상위 N건(기본값 5건)의 기사 요약을 이어붙여 단일 오디오로 렌더링해야 한다
...
- **TtsAudio**: 생성된 오디오 작업. 소유자 유형(ARTICLE/BRIEF)·참조 ID·음성 ID·상태·오디오 URL·재생 시간을 보유. 동일 (ownerType, refId, voiceId) 조합은 단 하나만 존재(캐시 키)
...
- **SC-004**: 데일리 브리핑(5건) 초회 생성이 요청 후 60초 이내 완료된다
...
- 브리핑 오디오는 기사별 요약을 이어붙인 단일 파일로 렌더링한다 (기사별 분리 파일 아님)
- 오디오 파일은 클라우드 오브젝트 스토리지에 저장되고 CDN을 통해 제공된다 (제공자는 플랜에서 결정)
- TTS 생성 타이밍(사용자 요청 시 온디맨드 vs. 설정된 briefTime에 사전 생성)은 플랜에서 결정한다
